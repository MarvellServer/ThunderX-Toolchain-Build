...
then
  docker cp ${container_id}:/root/rpmbuild/RPMS/aarch64/flang-9.0.0-${DATESTRING}.el8.aarch64.rpm ${BASE_DIR}/
elif [ "${DISTRO_NAME}" == "ubuntu" ]
then
  docker cp ${container_id}:/tmp/flang-9.0.0-${DATESTRING}-${DISTRO_NAME}-${DISTRO_VERSION}_arm64.deb ${BASE_DIR}/
fi
