...
then
  docker cp ${container_id}:/root/rpmbuild/RPMS/aarch64/llvm-10.0.1-${DATESTRING}.el8.aarch64.rpm ${BASE_DIR}/
elif [ "${DISTRO_NAME}" == "ubuntu" ]
then
  docker cp ${container_id}:/tmp/llvm-10.0.1-${DATESTRING}-${DISTRO_NAME}-${DISTRO_VERSION}_arm64.deb ${BASE_DIR}/
fi
